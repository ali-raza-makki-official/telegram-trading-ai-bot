...
<body>
  ...
  <script>
    ...
    async function loadChartData(tf) {
      try {
        ...
        if (data && data.candles && data.candles.length > 0) {
          ...
          if (overlayState.ema) {
            calculateAndPlotEMAs(liveCandles);
          }

          // Update Latest Header Price
    ...
    async function runLiveAnalysis() {
      ...
      pre.innerText = 'Analyzing XAU/USD 7-timeframe structure with Google Gemini...';
      try {
        ...
        pre.innerText = 'Analysis Error: ' + e.message;
      }
    }
