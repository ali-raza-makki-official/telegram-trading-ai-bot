  ...
  <script>
    ...
    async function loadChartData(tf) {
      ...
      try {
        ...
        const cloudUrl = 'https://mt-market-data-client-api-v1.london-a.agiliumtrade.ai/users/current/accounts/' + METAAPI_ACC_ID + '/historical-market-data/symbols/XAUUSDm/timeframes/' + tf + '/candles?startTime=' + startTime + '&limit=100';
        const cRes = await fetch(cloudUrl, { headers: { 'auth-token': METAAPI_TOKEN } });
        const cData = await cRes.json();
        if (Array.isArray(cData) && cData.length > 0) {
          liveCandles = cData.map(c => ({
