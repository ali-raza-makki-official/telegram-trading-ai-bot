 ...
 * Renders high-resolution TradingView-style dark charts with Order Blocks, FVGs, SL/TP levels.
 */
class ChartRenderer {
  static generateSMCChartSVG({
    ...
    const width = 1000;
    const height = 580;
    const padding = { top: 70, right: 90, bottom: 40, left: 30 };
    ...
    // Include SL/TP in price bounds if present
    ...
    const getY = (price) => padding.top + chartHeight - ((price - minPrice) / finalRange) * chartHeight;

    const candleWidth = Math.max(6, (chartWidth / count) * 0.65);
    ...
    if (ema20 && ema20.length > 1) {
      ...
      if (d) {
        emaSvg = `<path d="${d}" fill="none" stroke="#f59e0b" stroke-width="2" opacity="0.85" stroke-linecap="round" />`;
      }
    }
    ...
    if (setup.orderBlock && setup.orderBlock.top && setup.orderBlock.bottom) {
      ...
      const obH = Math.max(6, obBottom - obTop);
      const isBull = setup.orderBlock.type === 'BULLISH' || setup.bias === 'BULLISH';
      const obColor = isBull ? '#10b981' : '#ef4444';
      const obLabel = isBull ? 'BULLISH ORDER BLOCK' : 'BEARISH ORDER BLOCK';

      smcSvg += `
        <rect x="${padding.left + chartWidth * 0.3}" y="${obTop}" width="${chartWidth * 0.7}" height="${obH}" rx="4" fill="${obColor}" fill-opacity="0.15" stroke="${obColor}" stroke-dasharray="4,4" stroke-width="1.5" />
        <rect x="${padding.left + chartWidth * 0.3 + 8}" y="${obTop + 4}" width="150" height="18" rx="3" fill="#0f172a" fill-opacity="0.9" />
        <text x="${padding.left + chartWidth * 0.3 + 14}" y="${obTop + 17}" fill="${obColor}" font-family="'Inter', sans-serif" font-size="10" font-weight="700">📦 ${obLabel}</text>
      `;
    }
    ...
    if (setup.sl) {
      ...
      smcSvg += `
        ...
        <rect x="${width - padding.right + 6}" y="${slY - 10}" width="78" height="20" rx="4" fill="#ef4444" />
    ...
    if (setup.tp1) {
      ...
      smcSvg += `
        ...
        <rect x="${width - padding.right + 6}" y="${tp1Y - 10}" width="80" height="20" rx="4" fill="#10b981" />
    ...
    smcSvg += `
      ...
      <rect x="${width - padding.right + 6}" y="${curY - 10}" width="82" height="20" rx="4" fill="#3b82f6" />
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
  ...
  <g transform="translate(24, 20)">
    ...
    <rect x="320" y="0" width="160" height="34" rx="6" fill="${setup.bias === 'BULLISH' ? '#064e3b' : setup.bias === 'BEARISH' ? '#7f1d1d' : '#1e293b'}" />
    ...
    <!-- Broker / Engine Info -->
    <text x="${width - padding.right - 220}" y="22" fill="#64748b" font-family="'Inter', sans-serif" font-size="12">Exness MT5 • DeepSeek-V4-Pro</text>
  </g>
  ...
  <!-- Footer Watermark -->
  <text x="24" y="${height - 15}" fill="#475569" font-family="'Inter', sans-serif" font-size="11">Autonomous Gold AI Trading Agent • SMC / ICT Confluence Engine</text>
</svg>
`.trim();
  }
